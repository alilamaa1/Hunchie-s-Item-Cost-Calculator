...
const channels = {
  ...
  BATCH_DRAFT_CALCULATE: 'batch-draft:calculate',
...
contextBridge.exposeInMainWorld('itemCostApi', {
  ...
  listBatches: () => ipcRenderer.invoke(channels.BATCH_LIST),
  createBatch: (input) => ipcRenderer.invoke(channels.BATCH_CREATE, input),
  updateBatch: (id, input) => ipcRenderer.invoke(channels.BATCH_UPDATE, id, input),
  getBatch: (id) => ipcRenderer.invoke(channels.BATCH_DETAIL, id),
  deleteBatch: (id) => ipcRenderer.invoke(channels.BATCH_DELETE, id),
  calculateBatchDraft: (input) => ipcRenderer.invoke(channels.BATCH_DRAFT_CALCULATE, input),
