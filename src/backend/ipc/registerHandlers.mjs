...
export function registerIpcHandlers(ipcMain, services, contextFactory = () => ({})) {
  const handlers = {
    ...
    [IpcChannels.BATCH_LIST]: () => services.listBatches(contextFactory()),
    [IpcChannels.BATCH_CREATE]: (_event, input) => services.createBatch(input, contextFactory()),
    [IpcChannels.BATCH_UPDATE]: (_event, id, input) => services.updateBatch(id, input, contextFactory()),
    [IpcChannels.BATCH_DETAIL]: (_event, id) => services.getBatchById(id, contextFactory()),
    [IpcChannels.BATCH_DELETE]: (_event, id) => services.deleteBatch(id, contextFactory()),
    [IpcChannels.BATCH_DRAFT_CALCULATE]: (_event, input) => services.calculateBatchDraft(input, contextFactory()),
